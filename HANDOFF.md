...
**她还提了一个待办**：让 Claude 官方 app 里也能用上查岗 / 记忆这些。
方向是给 wu-home 加一个 **MCP 服务端**（现在只有客户端那一半），
然后在 claude.ai 里添加成自定义连接器。⚠️ 认证方式（OAuth？URL token？）
和手机端能不能添加，都还没确认 —— 别急着写，先让她在 claude.ai 设置里
找到「连接器」入口、看清楚要求再动手。
